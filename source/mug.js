...
export default class Mug {
  constructor(gl) {
    var ObjStr = document.getElementById("mug.obj").innerHTML;
    const mugData = new objLoader.Mesh(ObjStr);
    this.colors = [];
    this.pick = 0;
    this.fclr = [30 / 255, 1, 1];
    this.vertexAttributesData = new Float32Array(mugData.vertices);
    this.vertexIndices = new Uint16Array(mugData.indices);
    this.gl = gl;

    for (var j = 0; j < mugData.vertices.length / 3; j++) {
